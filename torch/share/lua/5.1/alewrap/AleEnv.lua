...
function alewrap.createEnv(romName, display, extraConfig)
    return alewrap.AleEnv(romName, extraConfig, display)
end
...
function Env:__init(romPath, extraConfig, display)

    if display == 0 then	-- Enable screen
	self.config = {
